...
import numpy as np
from src.custom.shapey import Shape, ShapeArray, plshow, g_lines
...
def method1(image, idx=""):
    ...
    h_thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, horizontal_kernel, iterations=1)
    if show:
    ...
    lines = cv2.HoughLinesP(img_bin_final, rho=1, theta=np.pi/180, threshold=100, minLineLength=100, maxLineGap=10)
   ...
   # Define kernel size and create kernel
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            ...
            kernel_image = img[max(0, i-kernel_size//2):min(img.shape[0], i+kernel_size//2+1),
    ...
    #         cv2.imshow('result', result)
    #         cv2.waitKey(175)
def method6(image, idx="", debug_level=0):
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    for i, cnt in enumerate(inv):
        ...
        cy = int(M["m01"] / M["m00"])
        # Add a text label to the image with the contour's index
        cv2.putText(merged, str(i), (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    for (i, cnt) in enumerate(cnts):
        ...
        contours.append(cnt)
    return shapes, contours
def method7(image, idx="", debug_level=0):
    if debug_level == 3:
        if idx+1 in [46]:
            debug_level=1
    img = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    ...
    edges = cv2.adaptiveThreshold(blur,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV,27,7)
    # Filter out all numbers and noise to isolate only boxes
    ...
    for line in filtered_lines:
        ...
        if abs(y2 - y1) < abs(x2 - x1):
            ...
            cv2.line(h_lines, (x1_extended, y1_extended), (x2_extended, y2_extended), 255, thickness=2)
        else:
            # Vertical line
            # Calculate the middle point of the line
            mid_point = ((x1 + x2) // 2, (y1 + y2) // 2)
            ...
            cv2.line(v_lines, (x1_extended, y1_extended), (x2_extended, y2_extended), 255, thickness=2)
    ...
    h_lines_dilated = cv2.dilate(h_lines, cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size)), iterations=5)
    v_lines_dilated = cv2.dilate(v_lines, cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size)), iterations=5)
    ...
    kernel_size = 3
    lines_eroded = cv2.erode(lines_combined, cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size)), iterations=7)
    if debug_level == 1:
